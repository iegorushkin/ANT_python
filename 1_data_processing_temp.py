...
input_folder = 'H:/CK_RAW_Z/1hour/temp_stations'
output_folder = 'H:/CK_PROCESSED_Z'
pics_path = 'H:/data_processing_pics/'
inventory_path = 'H:/Central_Kamchatka/Meta_data/inventory_temp_stations.xml'
start = time.time()
...
for file in file_list:
    ...
    if st[0].stats.station in mc_list:
        response_prefilter = pre_filt_mc
    elif st[0].stats.station in guralp_list:
        response_prefilter = pre_filt_guralp
    elif st[0].stats.station in bh_list:
        print("The current data is taken from a permanent station. Skip...")
        continue
    else:
